...
{
  if ((addr >= p->sz || addr + 4 > p->sz) || addr >= USERTOP) // add boundary for user code.
  {
    return -1;
  }
...
int fetchstr(struct proc *p, uint addr, char **pp)
{
  ...
  if (addr >= p->sz || addr >= USERTOP) // boundary for user code.
  {
    return -1;
  }
  if (p->pid > 1 && addr < PGSIZE)
...
int argptr(int n, char **pp, int size)
{
  ...
  if (i < PGSIZE || i > USERTOP) // check that pointer is within the boundaries of the stack and is not entering the User Code.
  {
    return -1;
  }
  *pp = (char *)i;
